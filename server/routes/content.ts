...
import { ContentResponse, CreateContentRequest, UpdateContentRequest } from "@shared/database";
...
export const createContent: RequestHandler = (req, res) => {
  try {
    ...
    if (!contentData.value?.en || !contentData.value?.vi) {
      return res.status(400).json({ error: 'Content value in both languages is required' });
    }
